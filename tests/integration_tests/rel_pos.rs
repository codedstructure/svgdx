...
"#;
    ...
    assert!(transform_str(rel_h_input).unwrap().contains(expected_rect));
...
"#;
    ...
    assert!(transform_str(rel_v_input).unwrap().contains(expected_rect));
}
...
"##;
    let expected_rect = r#"<rect x="30" y="20" width="20" height="60" id="z"/>"#;
    assert!(transform_str(rel_refid_input)
        .unwrap()
