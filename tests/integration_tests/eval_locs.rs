...
#[test]
fn test_loc_points() {
    let input = format!(r##"{RECT_SVG}<polyline points="#a~x2 1, #b@bl, #c~w 4" />"##);
    let expected_polyline = r#"<polyline points="5 1, 20 5, 5 4"/>"#;
    let output = transform_str_default(input).unwrap();
    assert_contains!(output, expected_polyline);

    let input = format!(r##"{RECT_SVG}<polygon points="#a~x2 1, #b@bl, #c~w 4" />"##);
...
#[test]
fn test_loc_path() {
    let input = format!(r##"{RECT_SVG}<path d="M #a~x2 1 #b@bl #c~w 4 z" />"##);
